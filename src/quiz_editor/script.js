...
function loadQuestion(value, index, array){
  ...
  question_type_combobox.onchange = function(){ questionTypeOnChange(value.id, this.value)};
  ...
  question_levels_container.id = "levels_container_" + value.id;
...
function saveQuestion(value, index, array){
  ...
  if (text_input.value != ""){
    $.ajax({
      ...
      type: 'PATCH',
      dataType: 'json',
      data: {"question_text": text_input.value, "question_type": type_input.value},
      success: function(data, textStatus, xhr) {0
        if(xhr.status === 200){
